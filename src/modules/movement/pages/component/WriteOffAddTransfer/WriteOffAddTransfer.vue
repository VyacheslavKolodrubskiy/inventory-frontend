<template>
  <div>
    ...
    <ATransfer
      ...
      :titles="[` ${$t('common.productRegistry')}`, ` ${$t('common.productsForWriteOff')}`]"
    ...
    <WriteOffAddHelpModal v-model:visible="state.isHelpModalVisible" />
  </div>
</template>

<script lang="ts" setup>
import type { LabelInValueType } from 'ant-design-vue/lib/vc-select/Select';
import WriteOffAddHelpModal from './WriteOffAddHelpModal.vue';
...
function onSubmit() {
  ...
  if (hasError)
    return message.error(t('validation.notEnoughAmountToWriteOff'));

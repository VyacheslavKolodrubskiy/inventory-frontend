<template>
  <QDrawer size="lg" :title="$t('warehouse.viewWarehouse')">
    <template v-if="$canUse('Admin')" #extra>
      <EditButton @click="goToModalEdit" />
    </template>

    <ASkeleton v-if="loadingState.isLoading.value" active />

    <template v-else-if="warehouse">
      <ARow class="flex-nowrap" :gutter="20">
        <ACol flex="200px">
          <!-- TODO fallback image -->
          <AImage
            fallback="data:image/png;base64,"
            :src="warehouse.image"
            :width="200"
          />
        </ACol>
        <ACol flex="auto">
          <ATypographyTitle :level="4">
            {{ warehouse.title }}
          </ATypographyTitle>

          {{ warehouse.description }}

          <ADescriptions class="mt-2">
            <ADescriptionsItem :label="$t('common.counterparty')">
              <PermissionLink permission="Admin" :to="{ name: 'Warehouse.Counterparty.View', params: { counterpartyId: warehouse.counterparty.id } }">
                {{ warehouse.counterparty.title }}
              </PermissionLink>
            </ADescriptionsItem>
          </ADescriptions>
        </ACol>
      </ARow>

      <ADivider>{{ $t('common.location') }}</ADivider>

      <ATypographyTitle :level="5">
        {{ $t('common.city') }} {{ warehouse.city.title }}
      </ATypographyTitle>

      <SelectMapPoint
        v-model:value="warehouse.location"
        :default-city="warehouse.city.title"
        readonly
        :zoom="13"
      />
    </template>
  </QDrawer>
</template>

<script setup lang="ts">
import type { Nullable, UrlParam } from '@qlt2020/frutils';
import { WarehouseRepository } from '../warehouse.repository';
import type { Warehouse } from '../warehouse.model';
import PermissionLink from '@/shared/components/PermissionLink.vue';
import { Executor } from '@/shared/network/executor';
import SelectMapPoint from '@/shared/components/form/SelectMapPoint.vue';
import { useGo } from '@/shared/composables/usePage';
import { useLoading } from '@/shared/composables/useLoading';

const route = useRoute();
const { goToModalEdit } = useGo();

const dataId = computed(() => route.params.warehouseId as string);

const warehouse = ref<Nullable<Warehouse>>(null);
const loadingState = useLoading();

onMounted(() => {
  readWarehouse(dataId.value);
});

function readWarehouse(dataId: UrlParam) {
  return Executor.run({
    request: WarehouseRepository.read(dataId),
    loadingState,
    onResult(data) {
      warehouse.value = data;
    },
  });
}
</script>
